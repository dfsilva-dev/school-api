...
class SerializerEstudanteTestCase(TestCase):
    ...
    def test_validacao_de_cpf_de_estudante(self):
        '''Teste que verifica a rotina de validação do cpf estudante'''
        invalidData = {
            'nome': 'Teste de Modelo',
            'email': 'testedemodelo@gmail.com',
            'cpf': '12345678900',
        ...
        invalidSerializer = EstudanteSerializer(data=invalidData)
        
        self.assertFalse(invalidSerializer.is_valid())
        self.assertIn('cpf', invalidSerializer.errors)
